...
function Toast({ message, type, onClose }) {
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3 rounded-lg border backdrop-blur-sm font-mono text-sm max-w-sm ${s[type]}`}
    >
...
function Card({ children, className = "" }) {
  return (
    <div
      className={`rounded-xl border border-white/5 bg-white/[0.02] ${className}`}
    >
      {children}
    </div>
  );
...
function Field({ label, children, hint }) {
  return (
    <div>
      ...
      {children}
      {hint && <p className="text-slate-600 text-xs mt-1 font-mono">{hint}</p>}
    </div>
  );
}
...
function Input({ className = "", ...p }) {
  return (
    <input
      className={`w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-white text-sm font-mono placeholder-slate-600 focus:outline-none focus:border-cyan-500/50 transition-colors ${className}`}
      {...p}
    />
  );
}
function Textarea({ className = "", ...p }) {
  return (
    <textarea
      className={`w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-white text-sm font-mono placeholder-slate-600 focus:outline-none focus:border-cyan-500/50 transition-colors resize-none ${className}`}
      {...p}
    />
  );
}
function Select({ className = "", children, ...p }) {
  return (
    <select
      className={`w-full bg-[#0f0f1a] border border-white/[0.08] rounded-lg px-3 py-2.5 text-white text-sm font-mono focus:outline-none focus:border-cyan-500/50 transition-colors ${className}`}
      {...p}
    >
      {children}
    </select>
...
function Btn({
  ...
  return (
    <button
      className={`inline-flex items-center gap-2 font-mono font-bold rounded-lg transition-all disabled:opacity-50 ${sz[size]} ${v[variant]} ${className}`}
      disabled={loading}
      {...p}
    >
...
function ImageUploader({ value, onChange, label = "Image" }) {
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch("/api/admin/sanity-asset", {
        method: "POST",
        body: fd,
      });
      const data = await res.json();
  ...
  return (
    <Field label={label}>
      <div
        ...
        className="relative border border-dashed border-white/15 rounded-lg overflow-hidden cursor-pointer hover:border-cyan-500/40 transition-colors group"
        style={{ minHeight: 100 }}
      >
        {previewUrl ? (
...
function S3Uploader({ value, onChange, productSlug }) {
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      alert("S3 upload failed: " + e.message);
    } finally {
      setUploading(false);
    }
  }
  return (
    <Field label="Product File (S3)" hint="ZIP file uploaded directly to S3">
      <div
        ...
        className="border border-dashed border-white/15 rounded-lg p-4 cursor-pointer hover:border-cyan-500/40 transition-colors"
      >
        {uploading ? (
          <div>
...
function RichTextEditor({ value = [], onChange }) {
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        if (node.textContent.trim()) {
          blocks.push({
            ...
            markDefs: [],
            children: [
              { _type: "span", _key: uid(), text: node.textContent, marks: [] },
            ],
          });
        }
        return;
  ...
  const btnClass =
    "px-2 py-1 rounded text-slate-400 hover:bg-white/10 hover:text-white transition-colors font-mono text-xs";
...
function PreviewImagesUploader({ value = [], onChange }) {
  ...
  async function handleFiles(files) {
    ...
    try {
      ...
      alert("Preview upload failed: " + e.message);
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <Field
      ...
      hint="Multiple images shown in product gallery"
    >
...
function ProductForm({
  ...
  async function handleSave(publish = false) {
    if (!form.name || !form.slug) {
      onToast("Name and slug required", "error");
      return;
    }
    if (publish && !form.lemonsqueezyVariantId) {
      onToast("Paste the LemonSqueezy Variant ID before publishing", "error");
      return;
    }
    ...
    try {
      ...
      if (form.algorithmicVariations)
        doc.algorithmicVariations = form.algorithmicVariations;
      if (form.totalFiles) doc.totalFiles = form.totalFiles;
      if (form.image) doc.image = form.image;
      if (form.previewImages?.length)
        doc.previewImages = form.previewImages.map((img) => ({
          ...img,
          _key: img._key || uid(),
        }));
      if (form.lemonsqueezyVariantId)
        doc.lemonsqueezyVariantId = form.lemonsqueezyVariantId;
      if (form.collectionId)
        doc.collection = { _type: "reference", _ref: form.collectionId };
      if (form.categoryId)
        doc.category = { _type: "reference", _ref: form.categoryId };
      if (form.subpackId)
        doc.subpack = { _type: "reference", _ref: form.subpackId };
      ...
      if (isEdit) {
        ...
        if (
          ...
          String(form.price) !== String(product.price)
        ) {
          await fetch("/api/admin/lemonsqueezy-variant", {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              variantId: form.lemonsqueezyVariantId,
              price: form.price,
            }),
          });
        }
  ...
  const filteredCats = categories.filter(
    (c) => !form.collectionId || c.collection?._id === form.collectionId,
  );
  const filteredSubs = subpacks.filter(
    (s) => !form.categoryId || s.category?._id === form.categoryId,
  );
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          <Card className="p-5 space-y-4">
            ...
            <Field label="Product Code">
              <Input
                ...
                placeholder="29-16-hs-ipiflattop"
              />
            </Field>
          ...
          <Card className="p-5 space-y-4">
            ...
            <Field
              ...
              hint='e.g. "dQw4w9WgXcQ" from youtube.com/watch?v=dQw4w9WgXcQ'
            >
              <Input
            ...
            <Field label="Decay Curve">
              <Input
                value={form.decayCurve}
                onChange={(e) => set("decayCurve", e.target.value)}
                placeholder="Linear, Exponential, Logarithmic"
              />
            </Field>
            <Field label="Modes">
              <Input
                value={form.modes}
                onChange={(e) => set("modes", e.target.value)}
                placeholder="Major and Minor, All 12 semitones"
              />
            </Field>
            <Field label="Algorithmic Variations (per tone)">
              <Input
                value={form.algorithmicVariations}
                onChange={(e) => set("algorithmicVariations", e.target.value)}
                placeholder="Wide, Reverse, MS, Oct"
              />
            </Field>
            <Field label="Total Files">
              <Input
                value={form.totalFiles}
                onChange={(e) => set("totalFiles", e.target.value)}
                placeholder="3,456 Impulse Response files"
              />
            </Field>
          </Card>
          <Card className="p-5 space-y-4">
            ...
            <Field label="Subpack">
              <Select
                value={form.subpackId}
                onChange={(e) => set("subpackId", e.target.value)}
              >
                <option value="">— None —</option>
                {filteredSubs.map((s) => (
        ...
        <div className="space-y-4">
          <Card className="p-5 space-y-4">
            ...
            <Field
              ...
              hint="Create the product in LemonSqueezy first, then paste the Variant ID here"
            >
              <Input
                value={form.lemonsqueezyVariantId}
                onChange={(e) => set("lemonsqueezyVariantId", e.target.value)}
            ...
            {form.lemonsqueezyVariantId ? (
              <div className="flex items-center gap-2 text-emerald-400 text-xs font-mono">
                <CheckCircle size={13} />
                Variant {form.lemonsqueezyVariantId} linked — ready to publish
              </div>
            ) : (
              <div className="rounded-lg border border-amber-500/25 bg-amber-500/5 p-3 space-y-2">
                ...
                <p className="text-slate-400 text-xs">
                  Create this product in LemonSqueezy first, then paste the
                  Variant ID above.
                </p>
                <a
          ...
          <Card className="p-5 space-y-4">
            ...
            />
            <PreviewImagesUploader
              value={form.previewImages}
              onChange={(v) => set("previewImages", v)}
            />
          </Card>
        </div>
      </div>
      <div className="flex justify-end gap-3 pt-2">
        ...
        <Btn
          ...
          title={
            !form.lemonsqueezyVariantId
              ? "Paste LemonSqueezy Variant ID first"
              : ""
          }
          className={
            !form.lemonsqueezyVariantId ? "opacity-50 cursor-not-allowed" : ""
          }
        >
          <CheckCircle size={14} />
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [p, c, cat, sub] = await Promise.all([
        sanityRead(
          `*[_type == "product"] | order(_createdAt desc) { _id, name, slug, productCode, price, percentage, lemonsqueezyVariantId, fileUrl, image, previewImages, youtubeVideoId, fileSize, fileFormat, decayCurve, modes, algorithmicVariations, totalFiles, collection->{ _id, name, slug, emoji }, category->{ _id, name, slug, collection->{ _id } }, subpack->{ _id, name, slug, category->{ _id } } }`,
        ),
        sanityRead(
          `*[_type == "collection"] | order(order asc) { _id, name, slug, emoji, order }`,
        ),
        sanityRead(
          `*[_type == "category"] | order(order asc) { _id, name, slug, collection->{ _id, name } }`,
        ),
        sanityRead(
          `*[_type == "subpack"] | order(order asc) { _id, name, slug, category->{ _id, name } }`,
        ),
      ]);
      setProducts(p || []);
      setCollections(c || []);
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2 rounded-lg border border-white/5 bg-white/[0.03] text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/40 font-mono"
        />
      </div>
      <Card>
        {loading ? (
              ...
              <thead>
                <tr className="border-b border-white/5">
                  {[
                    ...
                    <th
                      key={h}
                      className="text-left px-4 py-3 text-xs font-mono text-slate-500 font-normal"
                    >
                      {h}
                    </th>
                  ))}
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {p._id.startsWith("drafts.") ? (
                        <span className="text-xs px-2 py-0.5 rounded-full border font-mono bg-amber-500/10 text-amber-400 border-amber-500/25">
                          draft
                        </span>
                      ) : (
                        <span className="text-xs px-2 py-0.5 rounded-full border font-mono bg-emerald-500/10 text-emerald-400 border-emerald-500/25">
                          published
                        </span>
                      )}
                    </td>
...
function HierarchyForm({ type, item, parents, onSave, onCancel, onToast }) {
  ...
  const [form, setForm] = useState({
    ...
    order: item?.order ?? "",
    parentId:
      (type === "category" ? item?.collection?._id : item?.category?._id) || "",
  });
  const [saving, setSaving] = useState(null); // null | 'draft' | 'publish'
  function set(k, v) {
    setForm((f) => ({ ...f, [k]: v }));
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (type === "category" && form.parentId)
        doc.collection = { _type: "reference", _ref: form.parentId };
      if (type === "subpack" && form.parentId)
        doc.category = { _type: "reference", _ref: form.parentId };
      const mutations = isEdit
        ? [{ patch: { id: item._id, set: doc } }]
        : [{ create: { _id: `drafts.${uid()}`, ...doc } }];
      await sanityWrite(mutations);
      onToast(`${type} ${isEdit ? "updated" : "created"}!`, "success");
      onSave();
    } catch (e) {
      onToast("Save failed: " + e.message, "error");
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-lg">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-mono text-xl font-bold text-white capitalize">
...
function HierarchyPanel({ type, label, pluralLabel, onToast }) {
  ...
  const pq = {
    category: `*[_type == "collection"] | order(order asc) { _id, name, emoji }`,
    subpack: `*[_type == "category"] | order(order asc) { _id, name }`,
  };

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await sanityRead(q[type]);
      setItems(data || []);
      if (pq[type]) {
        const p = await sanityRead(pq[type]);
        setParents(p || []);
      }
    } catch (e) {
      onToast("Failed to load", "error");
  ...
  async function handleDelete(item) {
    setConfirm(null);
    try {
      await sanityWrite([{ delete: { id: item._id } }]);
      onToast(`${label} deleted`, "success");
      load();
    } catch (e) {
      onToast("Delete failed: " + e.message, "error");
    }
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.category && (
                      <div className="text-slate-600 text-xs mt-0.5">
                        {item.category.collection?.emoji}{" "}
                        {item.category.collection?.name} › {item.category.name}
                      </div>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  {item.productCount != null && (
                    <div className="text-center">
                      ...
                      <div className="text-slate-600 text-[10px]">products</div>
                    </div>
                  )}
                  <div className="flex gap-1">
                    <Btn
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500 text-sm">Trim Pulses · Live data</p>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            icon: FolderOpen,
            label: "Subpacks",
            value: stats.subpacks,
            c: "amber",
          },
        ].map(({ icon: Icon, label, value, c }) => (
          <Card key={label} className="p-5">
            <div
              className={`w-8 h-8 rounded-lg flex items-center justify-center mb-3 bg-${c}-400/10`}
            >
      ...
      <Card className="p-6">
        ...
        {[
          ...
          >
            ...
            <span
              className={`font-mono text-sm ${count > 0 ? "text-amber-400" : "text-emerald-400"}`}
            >
              {count}/{total}
            </span>
          </div>
...
function OrdersPanel({ onToast }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <Card className="p-4 border-red-500/20 bg-red-500/5 text-red-400 text-sm font-mono">
          {error}
        </Card>
      )}
      <Card>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="text-white text-xs">
                        {o.attributes?.user_name}
                      </div>
                      <div className="text-slate-500 text-xs">
                        {o.attributes?.user_email}
                      </div>
                    </td>
                    <td className="px-4 py-3 font-mono text-white text-xs">
                      €{((o.attributes?.total || 0) / 100).toFixed(2)}
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-xs px-2 py-0.5 rounded-full border font-mono bg-emerald-500/15 text-emerald-400 border-emerald-500/30">
                        {o.attributes?.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-400 text-xs font-mono">
                      {o.attributes?.created_at
                        ? new Date(o.attributes.created_at).toLocaleDateString()
                        : "—"}
                    </td>
                    <td className="px-4 py-3">
                      <a
                        href={`https://app.lemonsqueezy.com/orders/${o.id}`}
                        target="_blank"
                        rel="noopener noreferrer"
...
function S3Panel({ onToast }) {
  ...
  async function sign(key) {
    ...
    try {
      const res = await fetch(
        `/api/download?fileKey=${encodeURIComponent(key)}`,
      );
  ...
  return (
    <div className="space-y-6">
      ...
      {signedUrl && (
        <Card className="p-4 border-emerald-500/20 bg-emerald-500/5 flex items-center gap-2">
          <input
            readOnly
            value={signedUrl}
      ...
      {error && (
        <Card className="p-4 border-amber-500/20 bg-amber-500/5 text-amber-400 text-sm font-mono">
          {error}
        </Card>
      )}
      <Card>
    ...
    <div className="flex min-h-screen bg-[#0a0a12] text-white">
      <aside className="w-56 shrink-0 h-screen sticky top-0 flex flex-col border-r border-white/5 bg-[#07070f]">
        ...
        <nav className="flex-1 py-4 px-3 space-y-0.5">
          {NAV.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              onClick={() => setTab(id)}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all ${tab === id ? "bg-cyan-500/10 text-cyan-400 font-medium" : "text-slate-400 hover:text-slate-200 hover:bg-white/5"}`}
            >
              <Icon size={15} />
              {label}
            </button>
        ...
        <div className="px-4 py-4 border-t border-white/5">
          <a
            href="/"
            className="flex items-center gap-2 text-slate-500 hover:text-slate-300 text-xs font-mono"
          >
            <Globe size={12} />
            trimpulses.com
          </a>
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-8 py-10">{panels[tab]}</div>
      </main>
      {toast && (
        <Toast
          key={toast.id}
